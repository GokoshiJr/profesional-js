...
async function getTopMoviesInSequence() {
  // Las peticiones salen en secuencia
  const ids = await getTopMoviesIds();
  const movies = []
  for (const id of ids) { // parecido al for each
...
async function getTopMoviesInParallel() {
  // Las peticiones salen a la misma vez
  const ids = await getTopMoviesIds();
  const moviePromises = ids.map(id => getMovie(id));
  const movies = Promise.all(moviePromises); // espera a que todas de resuelvan
  // si una no se resuelve, lanza error
  return movies;
}
...
async function getFastestTopMovie() {
  const ids = await getTopMoviesIds();
  const moviePromises = ids.map(id => getMovie(id));
  const movie = Promise.race(moviePromises); // agarra la primera que se resuelva
  return movie;
}
